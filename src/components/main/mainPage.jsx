...
const TravelAnimation = () => {
  ...
  useEffect(() => {
    const carouselTimer = setInterval(nextSlide, 4000);
  ...
  const createFallbackSvg = (name) => {
    return `data:image/svg+xml,${encodeURIComponent(`
      <svg xmlns="http://www.w3.org/2000/svg" width="300" height="300">
        ...
        <text x="50%" y="50%" text-anchor="middle" dy=".3em" fill="#999" font-family="Arial" font-size="16">${name}</text>
      </svg>
    `)}`;
  };
  ...
  return (
    <div className="flex items-center justify-center" id='home' >
      <div className="relative w-full max-w-6xl bg-white overflow-hidden">
          ...
          <div className="relative w-full h-screen overflow-hidden">
            <div 
              ...
              style={{ 
                ...
                transitionDuration: isTransitioning ? '0s' : '800ms',
            ...
            >
              {imageConfigs.map((config, index) => (
                <div key={index} className="w-full h-full flex-shrink-0 relative">
                  ...
                  <div className="absolute bottom-6 left-6 bg-black/40 text-white px-4 py-2 rounded-xl backdrop-blur-md border border-white/10"
                    style={{
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none z-10">
              <div 
                className="backdrop-blur-[2px] rounded-2xl px-6 py-2"
                style={{
                  transitionProperty: 'all',
                  transitionDuration: '1000ms',
                  transitionTimingFunction: 'ease-out',
                  transitionDelay: '0s',
                  transform: isTypingComplete ? 'scale(1.05)' : 'scale(1)'
                }}
              >
                ...
                <div 
                  className="mt-8 h-1 bg-gradient-to-r from-transparent via-[#FF6B35] to-transparent rounded-full"
                  style={{
                    transitionProperty: 'all',
                    transitionDuration: '1000ms',
                    transitionTimingFunction: 'ease-out',
            ...
            <div className="absolute bottom-6 right-6 flex space-x-3 z-20">
              {imageConfigs.map((_, index) => (
                <button
                  ...
                  className={`
                    w-3 h-3 rounded-full transform
                    ${currentSlide === index 
                      ? 'bg-white scale-110' 
                      : 'bg-white/60'
                    }
            ...
            <button
              ...
              disabled={isTransitioning}
              className="absolute left-4 top-1/2 transform -translate-y-1/2 z-20 bg-black/30 text-white p-3 rounded-full backdrop-blur-md disabled:opacity-50"
              style={{
                transitionProperty: 'all',
                transitionDuration: '300ms',
                transitionTimingFunction: 'ease',
            ...
            <button
              ...
              disabled={isTransitioning}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 z-20 bg-black/30 text-white p-3 rounded-full backdrop-blur-md disabled:opacity-50"
              style={{
                transitionProperty: 'all',
                transitionDuration: '300ms',
                transitionTimingFunction: 'ease',
        ...
        {/* Desktop Layout (1128px and above) */}
        <div className="desktop-layout">
          ...
          <div 
            className="absolute top-[355px] left-[575px] transform -translate-x-1/2 -translate-y-1/2 text-center z-10 pointer-events-none"
            style={{ 
              opacity: isAnimating ? 1 : 0,
              transform: `translate(-50%, -50%) scale(${isAnimating ? 1 : 0.95})`,
              transitionProperty: 'opacity, transform',
              transitionDuration: '1500ms, 1000ms',
              transitionTimingFunction: 'cubic-bezier(0.25, 0.46, 0.45, 0.94)',
              transitionDelay: isAnimating ? '1s, 0s' : '0s, 0s'
            }}
          >
          ...
          <div 
            className="absolute top-[410px] left-[850px] transform -translate-x-1/2 -translate-y-1/2 text-center z-10 pointer-events-none"
            style={{ 
              opacity: isAnimating ? 1 : 0,
              transform: `translate(-50%, -50%) scale(${isAnimating ? 1 : 0.95})`,
              transitionProperty: 'opacity, transform',
