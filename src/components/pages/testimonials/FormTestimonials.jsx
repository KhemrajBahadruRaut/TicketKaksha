...
const FormTestimonials = () => {
  // XSS Protection utilities (moved inside component)
  ...
  const validateComment = (comment) => {
    // First check if it has actual content (not just whitespace)
  ...
  const validateField = (field, value) => {
    ...
    switch (field) {
      case "name":
        ...
        break;
      case "comment":
        // FIX: Only check if value exists, don't trim yet
        if (!value) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare real FormData for backend
    ...
    try {
      const response = await fetch(
        "https://ticketkaksha.com.np/Backend/testimonials/add_testimonials.php",
        // "http://localhost/TICKETKAKSHA/Backend/testimonials/add_testimonials.php",
        {
          method: "POST",
          body: submissionData, 
        }
      ...
      if (response.ok) {
        ...
        showErrorToast("Failed to submit testimonial.");
      }
    } catch (error) {
  ...
  return (
    <div className="bg-white rounded-lg max-w-xl mx-auto">
      ...
      <ToastContainer position="top-right" newestOnTop limit={2} />
